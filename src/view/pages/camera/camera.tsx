...
> {
  ...
  find = async (shot: string) => {
    ...
    const m = this.syncOCRData(fullText, title, diff, exScore);
  ...
  upload = async (ex: number, bpi: string | number, song: songData, lastEx: number) => {
    ...
    const rawImg = this.state.rawCamData;
    if (!song) return alert("楽曲データが見つかりませんでした");
    if (!rawImg) return alert("Error");
    const blob = this.toBlob(rawImg);
    ...
    navigator
      .share({
        text: `☆${song.difficultyLevel} ${song.title}${_prefixFromNum(song.difficulty, false)}\n` + `EXSCORE:${ex}(自己ベスト${px}) BPI:${bpi}`,
        files: [image],
      })
      .catch((error) => {
        console.log(error);
  ...
  syncOCRData = async (body: string, title: string[], diff: string, exScore: OCRExScore) => {
    ...
    this.setState({
      ocrId: json && json.id ? Number(json.id) : -1,
    });
