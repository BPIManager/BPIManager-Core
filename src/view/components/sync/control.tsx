...
import { isOlderVersion } from "@/components/common/versions";

class SyncControlScreen extends React.Component<
  { userData: any; toggleSending: () => void },
  {
    isLoading: boolean;
    scoreData: any;
    sentName: string;
...
> {
  ...
  toggleErrorSnack = (mes?: string | null) =>
    this.setState({
      ...
      return (
        <React.Fragment>
          ...
          >
            ...
            <p>
              現在選択中のIIDXバージョン({_currentStore()}
              )は過去のバージョンです。
              <br />
              このバージョンはアーカイブのダウンロードのみ実行可能です。
            </p>
          </Alert>
        </React.Fragment>
      );
