...
class Compare extends React.Component<{ intl: any }, S> {
  ...
  async componentDidMount() {
    ...
    const bpiMapper = (t: scoreData[]) =>
      t.map((item: scoreData) => item.currentBPI);
  ...
  handleChange =
    (target: "compareFrom" | "compareTo" | "displayMode") =>
  ...
  filter = (): compareData[] => {
    ...
    return full.filter(
      (data) =>
        options["level"].some(
          (item: string) => item === data.difficultyLevel
        ) &&
        options["difficulty"].some(
          (item: string) => diffs[Number(item)] === data.difficulty
        )
    );
  };
  ...
  async dataHandler(newState: S = this.state): Promise<void> {
    ...
    if (t === "MYREKIDAI") {
      // 自己歴代比較
      const rekidais = ([] as scoreData[]).concat(
        (await myBest("11")).scoreData,
        (await myBest("12")).scoreData
      );
      for (let i = 0; i < rekidais.length; ++i) {
        const current = rekidais[i];
        const songData = sdb.get(
          sdb.genTitle(current.title, current.difficulty)
        );
        const from = fData.find(
          (item) =>
            item.title === current.title &&
            item.difficulty === current.difficulty
        );
        if (!songData || !from) continue;
        const max = songData["notes"] * 2;

        calc.setData(max, songData["avg"], songData["wr"]);
        ...
        const tScore =
          displayMode === "exScore"
            ...
            ? percentager(current.exScore, max)
            : 0;

        const percentage = percentager(from["exScore"], max);
        const gap =
          Math.ceil(
            displayMode === "exScore"
              ? from["exScore"] - tScore
              : displayMode === "bpi"
              ? from["currentBPI"] - tScore
              : displayMode === "percentage"
              ? percentager(current["exScore"], max)
              : 0
          ) / 10000;
    ...
    if (t.indexOf("ARENA") > -1) {
      // COMPARE TO ARENARANK AVERAGE
      const arenaRank = t.replace("ARENA ", "");
      const tData = await this.getArenaAvgs(arenaRank);
      for (let i = 0; i < fData.length; ++i) {
        let tScore = 0;
        const fCurrent = fData[i];
        const tCurrent = tData.find(
          (item: any) =>
            item.title === fCurrent.title &&
            item.difficulty === fCurrent.difficulty
        );
        const songData = sdb.get(
          sdb.genTitle(fData[i]["title"], fData[i]["difficulty"])
        );

        if (fCurrent["currentBPI"] === Infinity) {
          continue;
        }
        if (!tCurrent || !tCurrent.average) {
          continue;
        }
        if (!songData) {
          continue;
        ...
        tScore =
          displayMode === "exScore"
            ? avg
            : displayMode === "bpi"
            ? calc.setPropData(songData, avg, isSingle)
            : displayMode === "percentage"
            ? percentager(avg, songData.notes * 2)
            : 0;
        ...
        const gap =
          Math.ceil(
            displayMode === "exScore"
              ? fData[i]["exScore"] - tScore
              : displayMode === "bpi"
              ? fData[i]["currentBPI"] - tScore
              : displayMode === "percentage"
              ? percentager(tData[0]["exScore"], songData.notes * 2)
              : 0
          ) / 10000;
        result.push({
          title: fData[i]["title"],
    ...
    for (let i = 0; i < fData.length; ++i) {
      let tScore = 0;
      if (fData[i]["currentBPI"] === Infinity) {
        continue;
      }
      const tData = await scores.getItem(
        fData[i]["title"],
        fData[i]["difficulty"],
        t,
        isSingle
      ...
      if (!tData || tData.length === 0) {
        if (t !== "BPI" && t !== "PERCENTAGE" && t !== "WR" && t !== "AVERAGE")
          continue;
        tScore = 0;
      } else {
        tScore =
          displayMode === "exScore"
            ? tData[0]["exScore"]
            : displayMode === "bpi"
            ? calc.setPropData(songData, tData[0]["exScore"], isSingle)
            : displayMode === "percentage"
            ? percentager(tData[0]["exScore"], max)
            : 0;
      }
      if (t === "WR") {
        tScore =
      ...
      if (t === "AVERAGE") {
        tScore =
          displayMode === "exScore"
            ...
            ? 0
            : displayMode === "percentage"
            ? percentager(songData["avg"], max)
            : 0;
      }
      if (t === "BPI") {
        tScore =
          displayMode === "exScore"
            ? calc.calcFromBPI(goalBPI, true)
            : displayMode === "bpi"
            ? goalBPI
            : displayMode === "percentage"
            ? percentager(calc.calcFromBPI(goalBPI, true), max)
            : 0;
      }
      if (t === "PERCENTAGE") {
        tScore =
          displayMode === "exScore"
            ? Math.ceil((songData["notes"] * 2 * goalPerc) / 100)
            : displayMode === "bpi"
            ? calc.setPropData(
                songData,
                Math.ceil((max * goalPerc) / 100),
                isSingle
              )
            : displayMode === "percentage"
            ? goalPerc
      ...
      const gap =
        Math.ceil(
          displayMode === "exScore"
            ? (fData[i]["exScore"] - tScore) * 10000
            : displayMode === "bpi"
            ? (fData[i]["currentBPI"] - tScore) * 10000
            : displayMode === "percentage"
            ? (percentage - tScore) * 10000
            : 0
        ) / 10000;
      result.push({
        title: fData[i]["title"],
  ...
  getArenaAvgs = async (arenaRank: string = "A1") => {
    ...
    const t = await fetch(
      "https://proxy.poyashi.me/bpim/api/v1/stats/getAllAvgs?" + params
    );
  ...
  sortedData = (): compareData[] => {
    ...
    const sortedData: compareData[] = this.filter()
      .sort((a, b) => {
        switch (orderTitle) {
          case 0:
          default:
          ...
          case 5:
            return (
              (b.scoreData.missCount || -1) - (a.scoreData.missCount || -1)
            );
          case 6:
            return b.scoreData.exScore - a.scoreData.exScore;
          case 7:
            return (
              b.scoreData.exScore / (b.songData.notes * 2) -
              a.scoreData.exScore / (a.songData.notes * 2)
            );
          case 8:
      ...
      <Container fixed className="commonLayout">
        <Grid container spacing={1} style={{ margin: "5px 0" }}>
          <Grid item xs={6}>
            <FormControl style={{ width: "100%" }}>
              <InputLabel>
                <FormattedMessage id="Compare.From" />
              </InputLabel>
              <Select
                value={compareFrom}
                onChange={this.handleChange("compareFrom")}
              >
                {versionTitles.map((item) => (
          ...
          <Grid item xs={6}>
            <FormControl style={{ width: "100%" }}>
              <InputLabel>
                <FormattedMessage id="Compare.To" />
              </InputLabel>
              <Select
                value={compareTo}
                onChange={this.handleChange("compareTo")}
              >
                {versionTitles.map((item) => (
                  <MenuItem key={item.num} value={item.num}>
                    {item.title}
                ...
                <MenuItem value={"MYREKIDAI"}>MY REKIDAI</MenuItem>
        ...
        <Grid container spacing={1} style={{ margin: "5px 0" }}>
          <Grid item xs={12}>
            <FormControl style={{ width: "100%" }}>
              <InputLabel>
                <FormattedMessage id="Compare.Display" />
              </InputLabel>
              <Select
                value={displayMode}
                onChange={this.handleChange("displayMode")}
              >
        ...
        <FilterByLevelAndDiff
          ...
          includePMButtons={true}
        />
        <CompareTable
          full={this.sortedData()}
          isLoading={isLoading}
          page={page}
