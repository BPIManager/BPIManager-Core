...
export default class fbArenaMatch {
  ...
  getLatency = async () => {
    try {
      ...
      const fixedTime = parseInt(
        String(timeobj.unixtime * 1000 + (endTime - sendTime) / 2),
        10
      );
  ...
  listenDetail = (docId: string, func: any) =>
    onSnapshot(this.detail(docId), func);
  ...
  list = async () => {
    ...
    return query(
      ...
      where(
        ...
        ">",
        new Date(
          timeFormatter(3, subtract(3, "hour", new Date().getTime() + offset))
        )
      ),
