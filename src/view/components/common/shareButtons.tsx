...
const getURL = (text: string, url?: string) =>
  ...
  }&text=${encodeURIComponent(text)}&hashtags=BPIM&related=BPIManager`;
...
const ShareButtons: React.FC<P> = ({ withTitle, url, text }) => (
  <Grid container spacing={1} justifyContent="center">
    ...
    <Grid item xs={2} style={{ display: "flex", justifyContent: "center" }}>
      <Link
        target="_blank"
        href={getURL(withTitle ? text || "BPIManagerを使っています:" : "", url)}
      >
        <TwitterIcon />
      </Link>
    </Grid>
...
export const ShareList: React.FC<P> = ({
  withTitle,
  url,
  text,
  disableSubHeader,
  ...
  const buttons = [
    ...
    },
    {
      ...
      onClick: () =>
        window.open(
          getURL(withTitle ? text || "BPIManagerを使っています:" : "", url)
        ),
    },
  ];
  return (
    <List
      ...
      subheader={
        disableSubHeader ? (
          ...
          <ListSubheader>プロフィールをシェア</ListSubheader>
        )
      }
    >
      {buttons.map((item, i) => {
...
export const getShareURL = (url: string, text: string) =>
  `https://twitter.com/intent/tweet?url=${
    url || window.location.href
  }&text=${encodeURIComponent(text || "")}&hashtags=BPIM&related=BPIManager`;
