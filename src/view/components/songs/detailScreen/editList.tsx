...
const EditList: React.FC<{
  ...
  song: songData;
  closeFunc?: () => void;
}> = ({ onlyList, song, closeFunc }) => {
  const [openMenu, setOpenMenu] = useState(false);
  const [successSnack, setSuccessSnack] = useState<{
    added: boolean;
    visible: boolean;
    target: string;
  }>({ added: true, visible: false, target: "" });
  ...
  const loadLists = async () => {
    ...
    setAllLists(
      p.reduce((groups: ListItemBody[], item: DBLists) => {
        groups.push({
          title: item.title,
          num: item.num,
    ...
    setAllSavedLists(
      q.reduce((groups: number[], item: any) => {
  ...
  const toggleListsMenu = async (willOpen: boolean = false) => setOpenMenu(willOpen);
    ...
    >
      <div style={{ margin: "10px 6px 0" }}>
        <Tooltip title="楽曲をリストに追加/削除">
          <IconButton style={{ margin: "0 6px 0" }} aria-haspopup="true" onClick={() => toggleListsMenu(true)} size="large">
            <PlaylistAddCheckIcon />
          </IconButton>
        </Tooltip>
        <SwipeableDrawer anchor="bottom" className="overlayDrawer" open={openMenu} onClose={() => toggleListsMenu(false)} onOpen={() => toggleListsMenu(true)}>
          {ListBody}
        </SwipeableDrawer>
      </div>
      <ShowSnackBar
        message={
          <span>
            {successSnack.added ? <FormattedMessage id="Details.FavButtonAdded" /> : <FormattedMessage id="Details.FavButtonRemoved" />}:{successSnack.target}
          </span>
        }
        variant="success"
        handleClose={() => setSuccessSnack({ visible: false, added: false, target: "" })}
        open={successSnack.visible}
