...
export default class aaaDiffCalc {
  ...
  async exec(diff: number, target: number = 0): Promise<CLOrigInt> {
    const songs: songData[] = (await this.songsDB.getAll()).filter(
      (item: songData) => item.difficultyLevel === String(diff)
    );
    ...
    for (let i = 0; i < songs.length; ++i) {
      ...
      const res = this.calc.setManual(
        s.wr,
        s.avg,
        s.notes,
        Math.ceil(s.notes * 2 * t),
        coef
      );
  ...
  rangeSort(results: CLOrigInt) {
    Object.keys(results).map((item) => {
      results[item] = results[item].sort(
        (a: CLOrigin, b: CLOrigin) => b.bpi - a.bpi
      );
      return 0;
    });
    return results;
