...
const ImportScreen: React.FC<P & RouteComponentProps> = ({ updateGlobal, match, history }) => {
  ...
  const [result, setResult] = useState<ImportResult>({ stateText: "", errors: [], updated: 0, updatedText: "" });
  ...
  const execute = async (forceText?: string) => {
    ...
    setSaving(false);
    setRaw("");
    setResult({ stateText: res.stateText, errors: res.errors, updated: res.updated, updatedText: res.updatedText });
  }
    ...
    <Container fixed className="commonLayout">
        ...
        <MakeAlert severity="success" title={<FormattedMessage id="Data.End" />}>
          <div style={{ width: "100%" }}>
            ...
            <Button
              ...
              variant="outlined"
              color="secondary"
              onClick={() => history.push("/history/" + timeFormatter(7, new Date()))}
              style={{ margin: "5px 0" }}>
      ...
      {result.errors.length > 0 &&
        <MakeAlert title="実行ログ" severity="error">
          {result.errors.map(item => <span key={item}>{item}<br /></span>)}
        </MakeAlert>
      }
      <AdsCard />
      ...
      {(uid === "") && (
        <MakeAlert title="ログインしていません" severity="warning">
          <p>
            Sync機能を用いることで、スコアデータをクラウドと同期することが可能です。<br />
      ...
      {(!_autoSync() && uid !== "") && (
        <MakeAlert title="ご存知ですか？" severity="warning">
          <p>
            設定画面より「Auto-sync」を有効にすることで、最新のスコアデータをクラウドと自動同期できます。<br />
  ...
  return (
    <React.Fragment>
      ...
      <Typography variant="caption" component="p" style={{ margin: "8px 0" }}>
        <Link color="secondary" onClick={toggle}>ブックマークレットでスコアを取り込む<br />(eAMUベーシック会員向け)</Link>
      </Typography>
      {open && (
        <div>
...
const MakeAlert: React.FC<{ title: string|React.ReactNode, children: React.ReactNode, severity: AlertColor }> = ({ severity, title, children }) => {
  return (
