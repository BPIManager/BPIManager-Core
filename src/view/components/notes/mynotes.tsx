      ...
      return (
        <Container fixed>
          <Alert severity="error">
            ...
            <p>
              Myノート機能は、Syncにログインすることでご利用いただけます。
              <br />
              <RLink to="/sync">
                <Link color="secondary">こちら</Link>
              </RLink>
              からログインしてください。
            </p>
          </Alert>
      ...
      <Container fixed>
            ...
            return (
              ...
              >
                <ListItemText
                  primary={
                    <span>
                      {data.songName + _prefixWithPS(data.songDiff, data.isSingle)}&nbsp;<small>{updatedTime(data.wroteAt.toDate())}</small>
                    </span>
                  }
                  secondary={note}
                />
        ...
        {myNotes.length === 0 && (
          <Alert severity="error">
            ...
            <p>
              あなたの知恵をノートとしてシェアしませんか？
              <br />
              <RLink to="/songs">
                <Link color="secondary">楽曲一覧</Link>
              </RLink>
              からノートを投稿したい楽曲をタップし、「NOTES」タブを選択して書き込みしてください。
            </p>
          </Alert>
        )}
              ...
              <div>
                ...
                {!Number.isNaN(score.currentBPI) && (
                  <div>
                    ...
                    <TextField multiline disabled fullWidth rows={4} value={this.state.text} variant="outlined" />
                  </div>
                )}
              </div>
            )}
