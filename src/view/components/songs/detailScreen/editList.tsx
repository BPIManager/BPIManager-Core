...
const EditList: React.FC<{
  ...
  const toggleFavLists = async (
    target: { title: string; num: number },
    willAdd: boolean
  ) => {
    try {
      const title = song ? song.title : "";
      const difficulty = song ? song.difficulty : "";
      const res = await new favLists().toggleLists(
        title,
        difficulty,
        target.num,
        willAdd
      ...
      if (res) {
        ...
        setAllSavedLists(
          willAdd
            ? allSavedLists.concat(target.num)
            : allSavedLists.filter((item: number) => item !== target.num)
        );
      }
    } catch (e: any) {
      console.log(e);
    ...
    >
          ...
          >
            {allLists.map((item: ListItemBody) => {
              ...
              return (
                <ListItem
                  button
                  onClick={() => toggleFavLists(item, !alreadyExists)}
                  key={item.num}
                >
                  <ListItemIcon>
                    {alreadyExists ? (
                  ...
                  <ListItemText
                    primary={`${item.title}(${item.length})`}
      ...
      <ShowSnackBar
        message={
          <span>
            {successSnack.added ? (
              <FormattedMessage id="Details.FavButtonAdded" />
            ) : (
              <FormattedMessage id="Details.FavButtonRemoved" />
            )}
            :{successSnack.target}
          </span>
        }
        variant="success"
        handleClose={() =>
          setSuccessSnack({ visible: false, added: false, target: "" })
        }
