...
const DetailedSongInformation: React.FC<P & { intl?: any }> = (props) => {
  ...
  const isModified = () => {
    ...
    return (
      !Number.isNaN(newData.bpi) || !Number.isNaN(newData.score) || (newData.clear !== -1 && newData.clear !== score.clearState) || (newData.miss !== -1 && newData.miss !== score.missCount) || (newData.memoModified && newData.memo !== song.memo)
    );
  };
  ...
  return (
    <Dialog id="detailedScreen" className={c === "dark" ? "darkDetailedScreen" : c === "light" ? "lightDetailedScreen" : "deepSeaDetailedScreen"} fullScreen open={isOpen} onClose={handleOpen} style={{ overflowX: "hidden", width: "100%" }}>
      <AppBar style={{ boxShadow: "none" }}>
        <Toolbar>
          <IconButton edge="start" color="inherit" onClick={() => handleOpen(false)} aria-label="close" size="large">
            <CloseIcon />
          </IconButton>
      ...
      <ShowSnackBar message={errorSnack.message} variant="warning" handleClose={() => setErrorSnack({ message: "", visible: false })} open={errorSnack.visible} autoHideDuration={3000} />
    </Dialog>
  );
};
