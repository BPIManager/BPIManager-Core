...
interface P {
  ...
  handleOpen: (
    flag: boolean,
    row?: any,
    willDeleteItems?: { title: string; difficulty: string } | null
  ) => void;
  willDelete?: boolean;
  firstView?: number;
}
...
const DetailedSongInformation: React.FC<P & { intl?: any }> = (props) => {
  ...
  const modifyData = (
    target: "score" | "bpi" | "clear" | "miss" | "memo",
    value: number | string
  ) => {
    if (!song) return;
    setNewData({
      ...newData,
      [target]: value,
      memoModified: target === "memo" && newData.memo !== song.memo,
    });
  };
  ...
  const handleScoreInput = async (
    ...
    return setNewData({
      ...newData,
      score: newScore,
      bpi: Math.floor(newBPI.bpi * 100) / 100,
    });
  };
  ...
  const saveAndClose = async () => {
    try {
      ...
      if (
        ...
        await scores.updateScore(score, {
          currentBPI: newData.bpi,
          exScore: newData.score,
          clearState: newData.clear,
          missCount: newData.miss,
        });
      }
      ...
      props.handleOpen(
        true,
        song,
        willDelete ? { title: score.title, difficulty: score.difficulty } : null
      );
    } catch (e: any) {
  ...
  return (
    ...
    >
      ...
      <ShowSnackBar
        ...
        variant="warning"
        handleClose={() => setErrorSnack({ message: "", visible: false })}
...
const DetailedTopHeader: React.FC<{
  ...
  const calcRank = () =>
    score
      ? `${calc.rank(
          !Number.isNaN(newData.bpi) ? newData.bpi : score.currentBPI
        )}`
      : "-";
  ...
  const showRank = (isBody: boolean): string => {
    ...
    const s: number = !Number.isNaN(newData.score)
      ? newData.score
      : score.exScore;
  ...
  return (
    <Grid
      ...
      className="detailsScreenTopGrid"
    >
      <Grid
        ...
        <Typography component="h4" variant="h4" color="textPrimary">
          ...
          {song.wr !== -1 && (
            <div>
              ...
              {Number.isNaN(score.currentBPI) && Number.isNaN(newData.bpi) && (
                <span>-</span>
              )}
            </div>
          )}
      ...
      >
        ...
        <Typography component="h4" variant="h4" color="textPrimary">
          ...
          {song.wr !== -1 && (
            <div>
              {(!Number.isNaN(score.currentBPI) ||
                !Number.isNaN(newData.bpi)) && <span>{calcRank()}</span>}
              {Number.isNaN(score.currentBPI) && Number.isNaN(newData.bpi) && (
                <span>-</span>
              )}
            </div>
