...
class SyncRivalScreen extends React.Component<P, S> {
  ...
  uploadExec = async (willAdd = true) => {
    ...
    await this.fbActions.syncUploadRival(
      await this.rivalListsDB.getAll(),
      willAdd,
      this.uid()
    );
    this.refreshData();
      ...
      this.setState({
        ...
        userData:
          ...
          (await new fbActions()
            ...
            .load()),
        syncData: p,
        notUploaded: (await this.rivalListsDB.getAll()).filter(
          (item) => uidList.indexOf(item.uid) === -1
        ),
      });
    });
  };
      ...
      <React.Fragment>
        <Paper style={{ padding: "15px" }}>
          ...
          <Menu
            ...
            onClick={() => this.handleToggle(0)}
          >
            <MenuItem onClick={() => this.uploadExec()}>
              この端末上のデータに置き換え
            </MenuItem>
            <MenuItem onClick={() => this.uploadExec(false)}>
              サーバー上のデータを削除
            </MenuItem>
          </Menu>
          <Menu
                ...
                <ListItem key={item.to.displayName}>
                  ...
                  <ListItemText
                    ...
                    secondary={timeFormatter(
                      3,
                      item.updatedAt ? item.updatedAt.toDate() : new Date()
                    )}
                  />
                  <ListItemSecondaryAction
                    onClick={() => this.deleteOne(item.to.uid)}
                  >
              ...
              <ListItem key={item.rivalName}>
                <ListItemAvatar>
                  <UserIcon
                    ...
                    altURL={getAltTwitterIcon(item, true)}
                  />
                </ListItemAvatar>
                <ListItemText
        ...
        <Alert severity="info" style={{ margin: "10px 0" }}>
          ...
          <p>
            注意点：
            <br />
            ・サーバー上からライバルを削除しても、端末内にライバルのデータは残ります。
            <br />
            ・端末からライバルを削除すると、サーバー上のライバルデータも削除されます。
            <br />
            ・v0.0.4.2以降のバージョンでは、ライバルを追加すると同時に自動でサーバーに同期されます。
            <br />
          </p>
        </Alert>
        <ShowSnackBar
