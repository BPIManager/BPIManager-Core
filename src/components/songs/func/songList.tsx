...
import {
  ...
  _isBetween,
  isBefore,
} from "@/components/common/timeFormatter";
...
export class songFuncInList extends songFuncCommon {
  ...
  evaluateRange = (): boolean => {
    ...
    return r === 0
      ? true
      : r === 1
      ? isSameDay(data.updatedAt)
      : r === 2
      ? isSameDay(data.updatedAt, subtract(1, "day"))
      : r === 3
      ? isSameWeek(data.updatedAt, new Date())
      : r === 5
      ? _isBetween(data.updatedAt, rb.from, rb.to)
  ...
  evaluateMode = (max: number): boolean => {
    ...
    return m === 0
      ? true
      : m === 1
      ? data.exScore / max < 2 / 3
      : m === 2
      ? data.exScore / max < 7 / 9 && 2 / 3 < data.exScore / max
      : m === 3
      ? data.exScore / max < 8 / 9 && 7 / 9 < data.exScore / max
      : m === 4
      ? data.exScore / max < 17 / 18 && 8 / 9 < data.exScore / max
      : m === 5
      ? true
      : m === 6
      ? data.clearState <= 3
