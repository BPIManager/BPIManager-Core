...
export default class CameraResult extends React.Component<
  Props,
  {
    defaultResult: any;
        ...
        <Container fixed>
                  ...
                  <div style={{ textAlign: "center" }}>
                    ...
                    {score && (
                      <small style={{ textAlign: "center" }}>
                        {untilDate(score.updatedAt)}前<br />
                        EX:{score.exScore}
                      </small>
                    )}
                    {!score && (
                ...
                <Grid item xs={6} style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", margin: "10px 0" }}>
                  <div style={{ textAlign: "center" }}>
                    ...
                    <Typography component="h4" variant="h4" color="textPrimary">
                      {rekidai && (
                        <span>
                          {exScore - rekidai.exScore > 0 && "+"}
                          {exScore - rekidai.exScore}
                        </span>
                      )}
                    ...
                    {rekidai && (
                      <small style={{ textAlign: "center" }}>
                        IIDX{rekidai.storedAt}で{untilDate(rekidai.updatedAt)}前<br />
                        自己歴代スコア:{rekidai.exScore}
                      </small>
                    )}
                    {!rekidai && (
                      <small style={{ textAlign: "center" }}>
              ...
              <Grid container spacing={3}>
                <Grid item xs={4} style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", margin: "10px 0" }}>
                  ...
                  <small style={{ textAlign: "center" }}>
                    スコアレート
                    <br />
                    {((exScore / (this.song().notes * 2)) * 100).toFixed(2)}%
                  </small>
                </Grid>
                <Grid item xs={4} style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", margin: "10px 0" }}>
                  <Typography component="h6" variant="h6" color="textSecondary">
                  ...
                  <small style={{ textAlign: "center" }}>
                    BPI{nextBPI}まで
                    <br />
                    あと{this.calc.calcFromBPI(nextBPI, true) - exScore}点
                  </small>
                </Grid>
                <Grid item xs={4} style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", margin: "10px 0" }}>
                  <Typography component="h6" variant="h6" color="textSecondary">
                  ...
                  <Typography component="h4" variant="h4" color="textPrimary">
                    {this.song().wr === -1 && <span>-</span>}
                    {this.song().wr !== -1 && <div>{this.calc.rank(bpi.bpi)}</div>}
                  </Typography>
                  <small style={{ textAlign: "center" }}>
                    皆伝上位
                    <br />
                    {((this.calc.rank(bpi.bpi) / this.calc.getTotalKaidens()) * 100).toFixed(1)}%
                  </small>
                </Grid>
              </Grid>
            </React.Fragment>
          ...
          {defaultResult.title.length === 0 && (
            <Alert severity="warning">
              楽曲名を取得できませんでした。
              <br />
              <Link onClick={this.dialogToggle} color="secondary">
                ここをクリックして手動で楽曲を選択
              </Link>
              するか、
              <Link onClick={this.props.retry} color="secondary">
                再度リザルト画面を撮影
              </Link>
              してください。
            </Alert>
          ...
          {defaultResult.title.length > 0 && (
            <FormControl style={{ width: "100%", marginTop: "10px" }}>
              ...
              <Select fullWidth value={currentSongTitle} onChange={this.changeSongTitle}>
              ...
              <FormHelperText>
                {defaultResult.title.length}件の候補があります /{" "}
                <Link onClick={this.dialogToggle} color="secondary">
                  楽曲を手動選択
                </Link>
              </FormHelperText>
          ...
          <FormControl style={{ width: "100%", marginTop: "10px" }}>
            ...
            <Select fullWidth value={currentDifficulty} onChange={this.changeSongDifficulty}>
          ...
          <ButtonGroup fullWidth>
            {!saved && (
              <Button startIcon={<CheckIcon />} fullWidth onClick={this.save} color="secondary" variant="contained">
                スコア保存
              </Button>
            )}
            {saved && (
              <Button startIcon={<ThumbUpIcon />} fullWidth disabled color="secondary" variant="contained">
                保存しました
              </Button>
            )}
            <Button startIcon={<IosShareIcon />} className="noTextButton" style={{ maxWidth: "25%" }} onClick={() => this.props.upload(exScore, bpi.error ? "-" : bpi.bpi, this.song(), score ? score.exScore : 0)} />
          </ButtonGroup>
          <Button startIcon={<ReplayIcon />} fullWidth onClick={this.props.retry} style={{ margin: "10px 0 20px 0" }}>
            もう一度撮影
          </Button>
          {id !== -1 && <VoteButton id={id} token={token} />}
        </Container>
      ...
      <React.Fragment>
        <div style={{ display: "flex", justifyContent: "center", flexDirection: "column", textAlign: "center", marginBottom: "30px" }}>
          <Alert icon={false} variant="outlined" severity="info">
            ...
            {this.state.progress === 2 && <p>ご協力ありがとうございました</p>}
            {this.state.progress === 1 && <Loader />}
            {this.state.progress === 0 && (
              <ButtonGroup fullWidth>
                <Button color="secondary" onClick={() => this.vote(1)} startIcon={<ThumbUpIcon />}>
                  正確
                </Button>
                <Button color="secondary" onClick={() => this.vote(0)} startIcon={<ThumbDownIcon />}>
                  不正確
                </Button>
              </ButtonGroup>
            )}
