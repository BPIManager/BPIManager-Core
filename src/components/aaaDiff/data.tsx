...
export interface CLOrigin {
  title: string;
  difficulty: string;
  bpi: number;
}
export interface CLBody extends CLOrigin {
  currentBPI: number;
  exScore: number;
}
...
export interface CLOrigInt {
  [key: string]: CLOrigin[];
}

export const AAADifficulty = async (diff = 12, target = 0) =>
  await new aaaDiffCalc().exec(diff, target);

export const getTable = async (
  targetLevel: number = 12,
  named: any,
  target: number = 0
) => {
...
export const named = async (
  ...
  const fil = (t: any) =>
    t
      .filter(
        (item: rivalScoreData | scoreData) =>
          item.difficultyLevel === String(targetLevel)
      )
      .reduce((groups: { [key: string]: any }, item: any) => {
  ...
  const full: scoreData[] = await db.getItemsBySongDifficulty(
    String(targetLevel)
  );
  return fil(full);
};
