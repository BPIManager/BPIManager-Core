...
export const myBest = async (
  targetLevel: string = "12",
  sort: number = 0,
  isDesc: boolean = true,
  ...
  const data = (await new scoresDB(isSingle).getAllVersions()).filter(
    (item) => item.difficultyLevel === targetLevel
  );
  for (let key in data) {
    const d = data[key];
    ...
    if (
      !scoreDataKeys[title] ||
      d["exScore"] > scoreDataKeys[title]["exScore"]
    ) {
  ...
  return {
    scoreData: apply(
      sort,
      isDesc,
      Object.keys(scoreDataKeys).reduce((group: scoreData[], item) => {
    ...
    scoreByVersion: Object.keys(v).reduce(
      (group: scoreByVersion[], item: string) => {
        group.push({
