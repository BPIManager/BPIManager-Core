...
const AAATable: React.FC<P> = ({ data }) => {
  ...
  return (
    <React.Fragment>
      ...
      {!loading && checks.sort((a, b) => b - a).map((key: number) => {
      ...
      {openFilterModal && <AAATableFilterModal
        target={targetGoal} changeTarget={changeTarget}
        closeModal={handleFilterModal} isChecked={isChecked} targetLevel={targetLevel} changeLevel={changeLevel} toggle={toggle}
        defaultPM={defaultPM} _default={defaultChecks} result={result} handleChange={handleChange}
      />}
    </React.Fragment>
  );
