...
export default class ScoreTable extends React.Component<Readonly<P>, S> {
  ...
  async componentDidMount() {
    ...
    for (let i = 0; i < s.length; ++i) {
      ...
      const song = songs.find(
        (item: songData) =>
          item.title === m.title &&
          difficultyDiscriminator(item.difficulty) === m.difficulty
      );
      if (song) {
        result[song["title"] + difficultyDiscriminator(song["difficulty"])] =
          song;
      }
    }
  ...
  showDetails = (row: withRivalData | null, song?: songData | null) =>
    this.setState({
      currentScoreData: row,
            ...
            .map((row: withRivalData) => {
              ...
              return (
                <Grid
                  ...
                  xs={12}
                  sm={6}
                  className="gridWithPad"
                  key={row.title + row.difficulty}
                >
                  <CardContent
                    style={{ padding: 0, cursor: "pointer" }}
                    onClick={() => this.showDetails(row, song)}
                  >
                    <Typography
                      component="div"
                    ...
                    <Grid
                      ...
                      style={{ margin: "5px 0", paddingRight: "10px" }}
                    >
                      {[0, 1].map((item: number) => {
                        ...
                        const bpi = [
                          this.bpiCalc(song, row.myEx),
                          this.bpiCalc(song, row.rivalEx),
                        ];
                        return (
                          <Grid
                            item
                          ...
                          >
                            ...
                            >
                              <span style={{ fontSize: "12px" }}>BPI</span>
                              <br />
                              <span>{bpi[item]}</span>
                            </p>
                          </Grid>
