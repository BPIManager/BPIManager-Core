...
export default class bpiCalculator {
  ...
  async calc(
    ...
    try {
      ...
      return {
        error: false,
        bpi: this.exec(),
        difficultyLevel: this.propData[0]["difficultyLevel"],
      };
    } catch (e: any) {
      console.log(e);
  ...
  set allTwelvesLength(val: number) {
    this._allTwelvesLength = val;
  }
  set allTwelvesBPI(val: number[]) {
    this._allTwelvesBPI = val;
  }
  ...
  totalBPI(): number {
    ...
    */
    ...
    let sum = 0,
      k = Math.log2(this._allTwelvesLength);
    if (k === 0) k = 1;
